...
export default function TimeoutFeedback({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
      <div className="bg-surface border border-border rounded-xl shadow-3d max-w-md w-full mx-4 overflow-hidden">
        ...
        <div className="bg-gradient-to-r from-rose-500/10 to-orange-500/10 border-b border-rose-500/20 px-6 py-4 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-neutral-500">Time limit exceeded</p>
          </div>
        </div>
        ...
        <div className="px-6 py-6 space-y-6">
          ...
          <div className="bg-background border border-border rounded-lg p-4">
            <h3 className="text-sm font-bold text-foreground mb-3">{currentProblem.title}</h3>
          ...
          <div className="space-y-3">
            ...
            <button
              onClick={onRetry}
              disabled={isLoading}
              className="w-full flex items-center justify-center gap-2 bg-primary hover:brightness-110 disabled:opacity-50 text-white font-bold py-3 rounded-lg transition-all active:scale-95 shadow-lg shadow-primary/20"
            >
            ...
            {!isFirstProblem && onEasier && (
              <button
                onClick={onEasier}
                disabled={isLoading}
                className="w-full flex items-center justify-center gap-2 bg-surface border border-border hover:bg-foreground/5 disabled:opacity-50 text-foreground font-bold py-3 rounded-lg transition-all active:scale-95 group shadow-sm"
              >
